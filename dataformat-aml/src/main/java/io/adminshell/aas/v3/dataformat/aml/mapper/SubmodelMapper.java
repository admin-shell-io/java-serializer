...
public class SubmodelMapper extends BaseMapper<Submodel> {
    ...
    protected void toInternalElement(Submodel value, AmlGenerator generator, MappingContext context) throws MappingException {
        ...
        if (value.getKind() == ModelingKind.TEMPLATE) {
            builder = builder.withRefBaseSystemUnitPath("AssetAdministrationShellSystemUnitClasses/" + context.getMappingProvider().getInternalElementNamingStrategy().getName(
                    value.getClass(),
                    value,
                    null));
        }
